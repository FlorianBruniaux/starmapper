...
// Discriminated union rather than `RepoStats | null`: collapsing it would erase the
// 400-vs-404 distinction that the route still has to serve.
//
// cacheControl travels in the result because the logic that computes it depends on
// `partial` and `source`, both internal to this function. Recomputing it in the route
// would duplicate that branch.
export type RepoStatsResult =
  | { ok: true; stats: RepoStats; cacheControl: string }
  | { ok: false; error: "invalid_params"; status: 400 }
  | { ok: false; error: "no_data"; status: 404 };

// Extracted from src/app/api/stats/[owner]/[repo]/route.ts so [owner]/[repo]/page.tsx can
// call it directly instead of fetching its own public URL. No try/catch here: the caller
// decides, the route logs and returns a 500, the page falls back to null.
export const computeRepoStats = async (
  owner: string,
  repo: string,
): Promise<RepoStatsResult> => {
  const key = validateOwnerRepo(owner, repo);
  if (!key) return { ok: false, error: "invalid_params", status: 400 };
  ...
  if (!mvRow) {
    try {
      ...
      if (!totals || Number(totals.total) === 0) {
        ...
        if (!badgeRow) return { ok: false, error: "no_data", status: 404 };
        joinTimedOut = true;
      } else {
  ...
  for (const { location, cnt } of locationRows) {
    ...
    if (country)
      countryCount.set(country, (countryCount.get(country) ?? 0) + n);
    if (city) cityCount.set(city, (cityCount.get(city) ?? 0) + n);
  }

  const companyCount = new Map(
    companyRows.map(
      ({ company, cnt }) => [company, Number(cnt)] as [string, number],
    ),
  );
  ...
  const finalCountryCount =
    countryCount.size > 0 ? countryCount.size : (badgeRow?.countryCount ?? 0);
  ...
  const stats: RepoStats = {
    ...
    topCities: [...cityCount.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 30),
    topCompanies: [...companyCount.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 30),
    topUsers: [],
    powerStargazers,
  ...
  return { ok: true, stats, cacheControl };
};
