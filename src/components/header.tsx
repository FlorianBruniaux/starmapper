...
type Props = {
  ...
  nav?: React.ReactNode;
  showNav?: boolean;      // renders Leaderboard link; use nav prop for custom content
  showToken?: boolean;
  hasToken?: boolean;
  onTokenClick?: () => void;
};
    ...
    <header className={`${positionCls} z-20 border-b border-border-subtle backdrop-blur-sm`}>
      <div className={`${widthCls} ${px} h-14 flex items-center justify-between`}>
        ...
        <div className="flex items-center gap-2">
          {backLink && (
            <Link href={backLink} className="text-muted hover:text-foreground transition-colors -ml-1 p-1">
          ...
          <LogoSvg />
          <Link href="/" className="text-foreground font-semibold text-sm hover:text-accent-blue transition-colors">
            StarMapper
          </Link>
          {afterLogo}
        </div>
