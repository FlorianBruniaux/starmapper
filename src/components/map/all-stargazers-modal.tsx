...
type AllStargazersModalProps = {
  ...
  setFilterDate: React.Dispatch<
    React.SetStateAction<"all" | "30d" | "90d" | "1y">
  >;
  setFlyTarget: React.Dispatch<
    React.SetStateAction<{ lat: number; lng: number; login: string } | null>
  >;
...
export const AllStargazersModal = ({
  ...
  const [allSort, setAllSort] = useState<{ key: SortKey; dir: 1 | -1 }>({
    key: "followers",
    dir: -1,
  });
  ...
  const filteredStargazers = useMemo(() => {
    ...
    if (filterFollowers > 0)
      list = list.filter((u) => u.followers >= filterFollowers);
    ...
    if (filterDate !== "all") {
      ...
      list = list.filter(
        (u) => u.starredAt && new Date(u.starredAt).getTime() >= cutoff,
      );
    }
    if (filterCompany)
      list = list.filter((u) =>
        u.company?.toLowerCase().includes(filterCompany.toLowerCase()),
      );
    if (filterCountry)
      list = list.filter((u) =>
        u.location?.toLowerCase().includes(filterCountry.toLowerCase()),
      );
    if (filterCity)
      list = list.filter((u) =>
        u.location?.toLowerCase().includes(filterCity.toLowerCase()),
      );
    return [...list].sort((a, b) => {
      if (allSort.key === "followers")
        return (b.followers - a.followers) * allSort.dir;
      if (allSort.key === "login")
        return a.login.localeCompare(b.login) * allSort.dir;
      ...
      if (allSort.key === "company")
        return (a.company ?? "").localeCompare(b.company ?? "") * allSort.dir;
      const la = a.location ?? "";
      const lb = b.location ?? "";
      return la.localeCompare(lb) * allSort.dir;
    });
  }, [
    deferredAllStargazers,
    deferredSearch,
    allSort,
    filterFollowers,
    filterMapped,
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      setTableVisibleCount(
        Math.max(5, Math.ceil(el.clientHeight / TABLE_ROW_H)),
      );
    });
    ro.observe(el);
  ...
  const tableVStart = Math.max(
    0,
    Math.floor(tableScrollTop / TABLE_ROW_H) - TABLE_OVERSCAN,
  );
  const tableVEnd = Math.min(
    filteredStargazers.length,
    tableVStart + tableVisibleCount + TABLE_OVERSCAN * 2,
  );
  ...
  const exportCsv = (rows: Record<string, unknown>[]) => {
    ...
    a.download = `stargazers-${owner}-${repo}.csv`;
    a.click();
  };
  ...
  const fetchAndExport = async () => {
    ...
    try {
      ...
      const detailMap = new Map(
        (data.users as UserDetail[]).map((u) => [u.login, u]),
      );
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-border-subtle flex-shrink-0">
        ...
        <button
          onClick={onClose}
          aria-label="Close all stargazers"
      ...
      <div className="px-5 py-2.5 border-b border-border-subtle flex-shrink-0 flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2 min-w-0">
          <span
            id="filter-followers-label"
            className="text-2xs text-muted whitespace-nowrap"
          >
            Min followers
          </span>
          <div
            role="radiogroup"
            aria-labelledby="filter-followers-label"
            ...
            {(["all", "mapped", "unmapped"] as const).map((v) => (
              ...
              >
                {v === "all" ? (
                  ...
                  <>
                    <span aria-hidden="true">📍</span> On map
                  </>
                ) : (
                  "No location"
                )}
        ...
        <div className="flex items-center gap-2">
          ...
          <div
            role="radiogroup"
            aria-labelledby="filter-starred-label"
            className="flex gap-1"
          >
        ...
        <input
          value={filterCompany}
          onChange={(e) =>
            startTransition(() => setFilterCompany(e.target.value))
          }
          placeholder="Company…"
      ...
      >
        {isSearchPending && (
          <div className="sticky top-0 left-0 right-0 z-20 flex items-center justify-center py-1 bg-surface/80 backdrop-blur-sm">
            <div className="flex items-center gap-2 text-2xs text-muted-subtle">
              <svg
                className="animate-spin motion-reduce:animate-none w-3 h-3"
                aria-hidden="true"
                viewBox="0 0 24 24"
                fill="none"
              >
        ...
        <table className="w-full text-xs border-collapse">
          <thead className="sticky top-0 bg-surface z-10">
            <tr className="border-b border-border-subtle">
              {csvEnabled && (
                <th className="px-3 py-2.5 w-8">
                  <input
                    ...
                    checked={
                      selected.size > 0 &&
                      selected.size === filteredStargazers.length
                    }
                    ref={(el) => {
                      if (el)
                        el.indeterminate =
              ...
              <th
                scope="col"
                className="px-3 py-2.5 text-left text-muted font-medium w-6 text-right"
              >
                #
              </th>
              <th
                ...
                className="px-3 py-2.5 text-left text-muted font-medium"
              >
                <button
              ...
              <th
                scope="col"
                aria-sort={
                  allSort.key === "followers"
                    ? allSort.dir === -1
                      ? "descending"
                      : "ascending"
                    : "none"
                }
                className="px-3 py-2.5 text-right text-muted font-medium"
              >
                <button
              ...
              <th
                scope="col"
                aria-sort={
                  allSort.key === "location"
                    ? allSort.dir === 1
                      ? "ascending"
                      : "descending"
                    : "none"
                }
                className="px-3 py-2.5 text-left text-muted font-medium hidden sm:table-cell"
              >
                <button
              ...
              <th
                scope="col"
                aria-sort={
                  allSort.key === "starredAt"
                    ? allSort.dir === -1
                      ? "descending"
                      : "ascending"
                    : "none"
                }
                className="px-3 py-2.5 text-left text-muted font-medium hidden md:table-cell"
              >
                <button
                  onClick={() => toggleSort("starredAt")}
                  className="flex items-center gap-1 hover:text-foreground"
                >
                  Starred{" "}
                  <span aria-hidden="true">
              ...
              <th
                ...
                className="px-3 py-2.5 text-left text-muted font-medium hidden lg:table-cell"
              >
                <button
              ...
              <th
                scope="col"
                className="px-3 py-2.5 text-left text-muted font-medium hidden xl:table-cell"
              >
                Links
              </th>
              ...
              <th
                scope="col"
                className="px-4 py-2.5 text-center text-muted font-medium"
              >
                Map
              </th>
            </tr>
          ...
          <tbody>
              ...
              return (
                <tr
                  ...
                  className={`border-b border-surface transition-colors ${
                    csvEnabled
                      ? selected.has(u.login)
                        ? "bg-surface-alt cursor-pointer"
                ...
                >
                  ...
                  <td className="px-3 py-2 text-muted-subtle text-right">
                    {i + 1}
                  </td>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="min-w-0">
                        ...
                        {u.name && u.name !== u.login && (
                          <div className="text-muted text-2xs truncate max-w-36">
                            {u.name}
                          </div>
                        )}
                        {u.bio && (
                          <div
                            className="text-muted-subtle text-2xs truncate max-w-36"
                            title={u.bio}
                          >
                            {u.bio}
                          </div>
                  ...
                  <td className="px-3 py-2 text-muted-subtle hidden md:table-cell tabular-nums">
                    {u.starredAt
                      ? new Date(u.starredAt).toLocaleDateString()
                      : "—"}
                  </td>
                  ...
                  <td
                    className="px-3 py-2 hidden xl:table-cell"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <div className="flex items-center gap-2">
                      {u.email && (
                      ...
                      {u.twitter_username && (
                        <a
                          ...
                          title={`@${u.twitter_username}`}
                          aria-label={`Twitter: @${u.twitter_username}`}
                          className="text-muted hover:text-accent-blue transition-colors"
                          target="_blank"
                          rel="noopener noreferrer"
                        >
            ...
            {tablePadBottom > 0 && (
              <tr style={{ height: tablePadBottom }}>
                <td colSpan={10} style={{ padding: 0 }} />
              </tr>
            )}
          </tbody>
        ...
        {filteredStargazers.length === 0 && !isSearchPending && (
          <div className="text-center text-muted-subtle text-xs py-12">
            No results for &ldquo;{allSearch}&rdquo;
          </div>
        )}
      </div>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-border-subtle bg-background rounded-b-2xl flex-shrink-0">
          <span className="text-xs text-muted">
            ...
            <button
              ...
              className="ml-3 text-muted-subtle hover:text-muted"
            >
              ✕ Clear
            </button>
          </span>
          <div className="flex gap-2">
            ...
            <button
              onClick={() =>
                exportCsv(allStargazers.filter((u) => selected.has(u.login)))
              }
              className="bg-surface-alt border border-border rounded-lg px-3 py-1.5 text-xs text-muted hover:text-foreground transition-colors"
            >
              ↓ Export CSV
            ...
            >
              {fetching
                ? "Fetching…"
                : `↓ Fetch details + CSV (${selected.size})`}
            </button>
          </div>
        </div>
      )}
