...
const ORGANIC_ENABLED =
  process.env.NEXT_PUBLIC_ORGANIC_SCORE_ENABLED === "true";
const DEPENDENTS_ENABLED =
  process.env.NEXT_PUBLIC_DEPENDENTS_ENABLED === "true";
...
type ScanStatus =
  "idle" | "loading" | "waiting" | "done" | "cached" | "refreshing" | "error";
...
const TopPanelInner = ({
  ...
  const [organicOverride, setOrganicOverride] = useState<RepoOrganic | null>(
    null,
  );
  ...
  const mappingPct =
    displayTotal > 0 ? Math.round((points.length / displayTotal) * 100) : 0;
  const locationCount = useMemo(
    () =>
      new Set(
        points
          .map((p) => {
            const s = p.location?.split(",").pop()?.trim();
            return s && isCountry(s) ? normalizeCountry(s) : null;
          })
          .filter(Boolean),
      ).size,
    [points],
        ...
        {!collapsed && (
          <>
            ...
            <div className="mt-3 border border-border-subtle rounded-lg overflow-hidden">
              <div
                className={`grid divide-x divide-border-subtle ${showOrganic ? "grid-cols-4" : "grid-cols-3"}`}
              >
                {/* Mapped */}
              ...
              >
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="relative group/tip">
                    <Info
                      size={9}
                      className="text-muted-subtle opacity-60"
                      aria-hidden="true"
                    />
                    <span className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-52 rounded-lg bg-surface border border-border px-3 py-2 text-xs text-foreground leading-relaxed shadow-xl opacity-0 group-hover/tip:opacity-100 transition-opacity duration-150 z-50 whitespace-normal font-normal normal-case tracking-normal">
                      These stargazers are fully indexed but haven&apos;t set a
                      location in their GitHub profile, so they can&apos;t be
                      placed on the map.
                    </span>
                  </span>
                </div>
              ...
              points.length > 0 && (
                <div className="mt-2.5">
                  ...
                  <div className="text-xs text-muted mt-0.5 text-center flex items-center justify-center gap-1">
                    <span>
                      {mappingPct}% mapped · {points.length.toLocaleString()} of{" "}
                    ...
                    <span className="relative group/tip">
                      ...
                      />
                      <span className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-52 rounded-lg bg-surface border border-border px-3 py-2 text-xs text-foreground leading-relaxed shadow-xl opacity-0 group-hover/tip:opacity-100 transition-opacity duration-150 z-50 whitespace-normal font-normal normal-case tracking-normal">
                        All {displayTotal.toLocaleString()} stargazers are
                        indexed. Only those who&apos;ve set a location in their
                        GitHub profile can be placed on the map.
                      </span>
                    </span>
                  </div>
                </div>
                      ...
                      {!hasToken && (
                        <span className="relative group/tip">
                          ...
                          <span className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-44 rounded-lg bg-surface border border-border px-3 py-2 text-xs text-foreground leading-relaxed shadow-xl opacity-0 group-hover/tip:opacity-100 transition-opacity duration-150 z-50 whitespace-normal font-normal normal-case tracking-normal">
                            Add a free GitHub token for faster scanning. No
                            login needed.
                          </span>
                        </span>
                      )}
                ...
                tooltip rather than removed. */}
                  <span className="relative group/tip inline-flex">
                    ...
                    <span className="pointer-events-none absolute bottom-full right-0 mb-2 w-60 rounded-lg bg-surface border border-border px-3 py-2 text-xs text-foreground leading-relaxed shadow-xl opacity-0 group-hover/tip:opacity-100 transition-opacity duration-150 z-50 whitespace-normal font-normal normal-case tracking-normal">
                      Rescan is paused. GitHub restricted public access to
                      stargazer lists on July 23 2026, so a rescan would return
                      nothing. The current map stays available.
                    </span>
                  </span>
                </div>
              </div>
            ...
            {/* ── Find a stargazer ──────────────────────────────────────────────── */}
            <div
              data-tour="map-find-stargazer"
              className="mt-2.5 pt-2.5 border-t border-border-subtle"
            >
              ...
              <div className="mt-2">
                {storedUsername && !askingUsername ? (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-2xs text-muted-subtle truncate">
                      @{storedUsername}
                    </span>
                    <button
                      onClick={() => {
                        onSetUsername("");
                        setAskingUsername(false);
                      }}
            ...
            <div
              className={`mt-2 pt-2 border-t border-border-subtle grid gap-2 ${DEPENDENTS_ENABLED ? "grid-cols-2" : "grid-cols-1"}`}
            >
              <a
