...
import { GET } from "@/app/api/trending/repos/route";
...
describe("GET /api/trending/repos", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  ...
  describe("response shape", () => {
    it("returns 200 with repos and meta.total — no mapPoints", async () => {
      const res = await GET();
      expect(res.status).toBe(200);
      const json = await res.json();
      expect(Array.isArray(json.repos)).toBe(true);
      expect(typeof json.meta.total).toBe("number");
      expect(json.mapPoints).toBeUndefined();
    });
    ...
    it("marks repos without cache entry as hasMap=false", async () => {
      mockCacheFindMany.mockResolvedValue([]);
      const json = await (await GET()).json();
      expect(json.repos[0].hasMap).toBe(false);
    });

    it("marks repos with cache entry as hasMap=true", async () => {
      mockCacheFindMany.mockResolvedValue([{ owner: "vercel", repo: "next.js" }]);
      const json = await (await GET()).json();
      expect(json.repos[0].hasMap).toBe(true);
    });

    it("does not request points blob from stargazer_cache", async () => {
      await GET();
      const callArgs = mockCacheFindMany.mock.calls[0]?.[0] as { select?: Record<string, unknown> };
      expect(callArgs?.select?.points).toBeUndefined();
    });
  });
  ...
  describe("Cache-Control", () => {
    ...
    it("includes stale-while-revalidate=86400", async () => {
      const res = await GET();
