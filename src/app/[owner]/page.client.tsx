...
export default function UserPage({
  ...
  const [repoSort, setRepoSort] = useState<
    "stars_desc" | "stars_asc" | "name_asc" | "name_desc"
  >("stars_desc");
  ...
  useEffect(() => {
    ...
    fetch(`/api/user-repos?username=${owner}`, { headers })
      .then(async (r) => {
        ...
        if (!r.ok) {
          setLoadErr(data.error ?? "Failed to load repos");
          setStep("load-error");
          return;
        }
  ...
  const patchQueue = useCallback((name: string, patch: Partial<QueueItem>) => {
    setQueue((q) =>
      q.map((item) => (item.name === name ? { ...item, ...patch } : item)),
    );
  }, []);
  ...
  const handleStart = useCallback(async () => {
    ...
    for (const repo of toScan) {
      if (abortRef.current) {
        patchQueue(repo.name, { status: "error", err: "Cancelled" });
        continue;
      }
        ...
        if (totalCount > 0) {
          (async () => {
            try {
              ...
              const slim: SlimPoint[] = allPoints.map(
                ({ bio: _b, avatarUrl: _av, ...rest }) => rest,
              );
  ...
  const totalMapped = queue.reduce(
    (s, q) => s + (q.status === "done" ? q.mapped : 0),
    0,
  );
  const totalCountries = queue.reduce(
    (s, q) => s + (q.status === "done" ? q.countries : 0),
    0,
  );
              ...
              <div className="flex items-center gap-4 mb-8">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-muted text-sm">
                    {userInfo.publicRepos} public repos
                  </p>
                  <Link
              ...
              {!hasToken && (
                <div className="flex items-center gap-2 bg-accent-orange/10 border border-accent-orange/25 rounded-lg px-4 py-2.5 mb-6">
                  ...
                  <p className="text-xs text-muted">
                    ...
                    batch scan will hit rate limits fast.{" "}
                    <button
                      onClick={() => setTokenOpen(true)}
                ...
                {filtered.length === 0 ? (
                  ...
                  <div className="divide-y divide-border-subtle">
                    {filtered.map((r) => {
                      ...
                      const isDisabled =
                        !isSelected && selected.size >= MAX_BATCH;
                      return (
                        <label
                          key={r.name}
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              {r.fork && (
                                <span className="text-2xs text-muted border border-border rounded px-1.5 py-0.5 shrink-0">
                                  Fork
                                </span>
                              )}
                              {isCached && (
                                <span className="text-2xs text-accent-green border border-accent-green/30 rounded px-1.5 py-0.5 shrink-0">
                                  Cached
                                </span>
                              )}
                            </div>
                            {r.description && (
                              <p className="text-muted-subtle text-xs truncate mt-0.5">
                                {r.description}
                              </p>
                            )}
                          </div>
                          <div className="flex items-center gap-3 shrink-0 text-xs text-muted">
                            ...
                            <span className="flex items-center gap-1">
                              <svg
                                ...
                                fill="currentColor"
                                aria-hidden="true"
                                className="text-accent-orange/70"
                              >
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-muted text-xs mt-0.5">
                    {doneCount} / {queue.length} repos · {fmt(totalMapped)}{" "}
                    mapped
              ...
              <div className="border border-border rounded-lg overflow-hidden mb-6">
                <div className="divide-y divide-border-subtle">
                  {queue.map((item) => {
                    const pct =
                      item.total > 0
                        ? Math.round((item.processed / item.total) * 100)
                        : 0;
                    return (
                      <div
                        key={item.name}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {item.status === "rate-limited" && (
                              <span className="text-2xs text-accent-orange shrink-0">
                                Rate limited, retrying…
                              </span>
                            )}
                            {item.status === "error" && item.err && (
                              <span className="text-2xs text-accent-red truncate">
                                {item.err}
                              </span>
                            )}
                          </div>
                          {(item.status === "scanning" ||
                            ...
                            item.total > 0 && (
                              <div className="mt-1.5 flex items-center gap-2">
                                ...
                                <span className="text-2xs text-muted-subtle shrink-0">
                                  {fmt(item.processed)} / {fmt(item.total)}
                                </span>
                              </div>
                            )}
                          {item.status === "done" && (
                            <p className="text-muted-subtle text-xs mt-0.5">
                              {fmt(item.mapped)} mapped · {item.countries}{" "}
                              countries
                            </p>
                          )}
                        </div>
