...
import type { CountryMapData } from "@/app/api/devs/in/[country]/route";
...
const cellsToPoints = (data: CountryMapData): StargazerPoint[] =>
  data.cells.map((c) => ({
    login: c.topLogin,
    name: `${c.count.toLocaleString()} developers`,
...
const MappedBadge = ({ count }: { count: number }) => (
  ...
  >
    ...
    <span className="hidden sm:inline">{count.toLocaleString()} mapped</span>
...
export default function DevsCountryClient({ initialSlug }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/devs/in/${encodeURIComponent(initialSlug)}`, { signal: ctrl.signal })
      .then(async (res) => {
        if (res.status === 404) {
          setNotFound(true);
          return;
        ...
        const json = (await res.json()) as CountryMapData;
        setData(json);
      })
      .catch((e) => {
  ...
  const countryName = slugToCountry(initialSlug);
  const maxLangCount = data && data.topLanguages.length > 0 ? data.topLanguages[0].count : 1;
      ...
      <div className="flex flex-col items-center justify-center min-h-screen gap-6 bg-background text-foreground px-4">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-muted text-sm">
            No developer data available for{" "}
  ...
  return (
    <div className="flex flex-col h-screen bg-background text-foreground overflow-hidden">
      <Header
        ...
        afterLogo={
          <>
            <span className="text-border-subtle shrink-0 select-none" aria-hidden="true">
              /
            </span>
            <Link
              href="/devs"
            ...
            <span className="text-foreground text-sm font-medium truncate max-w-40">
              {countryName}
            </span>
          </>
        }
        ...
        projectionButton={
          <button
            ...
            aria-label={
              mapProjection === "globe" ? "Switch to 2D flat map" : "Switch to 3D globe"
            }
          >
            {mapProjection === "globe" ? (
              <svg
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 bg-surface-alt">
              <div className="relative size-16">
                ...
                <div className="absolute inset-2 rounded-full border border-border-subtle animate-pulse motion-reduce:animate-none [animation-delay:150ms]" />
                <svg
                  width="32"
                  height="32"
                  viewBox="0 0 24 24"
            ...
            <div className="flex flex-col items-center justify-center h-full gap-5 bg-surface-alt px-6 text-center">
              ...
              <div className="flex flex-col gap-1.5">
                <p className="text-foreground font-semibold">
                  No data yet for{" "}
                  <span className="text-accent-blue">{countryName}</span>
                </p>
                <p className="text-muted text-sm">Backfill is in progress. Check back soon.</p>
              </div>
              <Link href="/devs" className="text-accent-blue text-sm hover:underline">
        ...
        {data && data.topLanguages.length > 0 && (
          ...
          >
            ...
            <ul className="flex-1 py-1" role="list">
              {data.topLanguages.map((l, index) => {
                const pct = Math.round((l.count / maxLangCount) * 100);
                const langSlug = languageToSlug(l.lang);
                return (
                  <li key={l.lang} className="group px-3 py-2">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Link
                        href={`/devs/${langSlug}`}
                        className="text-foreground text-xs truncate flex-1 group-hover:text-accent-blue transition-colors"
                      >
                        {l.lang}
                      </Link>
