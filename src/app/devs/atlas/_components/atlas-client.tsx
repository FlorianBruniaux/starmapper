...
type Props = {
  initialData: AtlasDominantData | null;
};

export const AtlasClient = ({ initialData }: Props) => {
  ...
  const legend = useMemo(() => (initialData ? computeLegend(initialData.countries) : []), [initialData]);
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            ...
            {totalCountries > 0 && (
              <div className="flex items-center gap-3 shrink-0 sm:pt-1">
                ...
                {initialData?.meta.minDevsThreshold && (
                  <div className="flex items-center gap-2 bg-surface border border-border-subtle rounded-lg px-3 py-2">
                    <span className="text-lg font-bold text-foreground tabular-nums">{initialData.meta.minDevsThreshold}</span>
          ...
          <div className="mt-3 bg-surface border border-border-subtle rounded-xl px-4 py-3">
            <div className="flex flex-wrap items-center justify-between gap-y-3">
              ...
              <p className="text-xs text-muted-subtle tabular-nums shrink-0">
                Updated {initialData ? formatDate(initialData.meta.generatedAt) : "—"}
              </p>
            </div>
          </div>
        )}
