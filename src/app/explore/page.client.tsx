...
const ReposBadge = ({ login, count }: { login: string; count: number }) => {
  ...
  const url = open
    ? `/api/explore/user-repos?login=${encodeURIComponent(login)}`
    : null;
      ...
      {open &&
        createPortal(
          ...
          >
            ...
            {loading && !data && (
              <div className="px-3 py-3 text-xs text-muted-subtle text-center">
                Loading…
              </div>
            )}
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          {r.language && (
                            <span className="text-xs text-muted-subtle flex-shrink-0">
                              {r.language}
                            </span>
                          )}
                        </div>
                        {r.description && (
                          <div className="text-xs text-muted truncate mt-px">
                            {r.description}
                          </div>
                        )}
                      </div>
...
const UserListSkeleton = ({ rows = PAGE_SIZE }: { rows?: number }) => (
  <div>
      ...
      {Array.from({ length: rows }).map((_, i) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0 space-y-1.5">
            ...
            />
            <div
              className="h-2.5 rounded bg-surface-alt animate-pulse opacity-60"
    ...
    <div
      className="flex items-center justify-between mt-5 pt-4 border-t border-border-subtle"
      aria-hidden="true"
    >
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/explore/autocomplete?q=${encodeURIComponent(value)}`,
        );
        if (!res.ok) return;
...
type ExplorePageProps = {
  initialSummary?: ExploreSummary | null;
};

export default function ExplorePage({ initialSummary }: ExplorePageProps = {}) {
  const { theme } = useTheme();
  const mapStyleUrl =
    theme === "light"
      ? MAP_STYLE_LIGHT(JAWG_TOKEN)
      : MAP_STYLE_DARK(JAWG_TOKEN);
  ...
  const [minFollowers, setMinFollowers] = useState<number>(() =>
    typeof window !== "undefined"
      ? Math.max(
          0,
          parseInt(
            new URLSearchParams(window.location.search).get("flw") ?? "0",
            10,
          ),
        )
      : 0,
  ...
  // Summary (loaded once; initialSummary bypasses the client-side fetch when server pre-fetched it)
  const [summary, setSummary] = useState<ExploreSummary | null>(
    initialSummary ?? null,
  );
  ...
  const [mapMode, setMapMode] = useState<"choropleth" | "heatmap">(() =>
    typeof window !== "undefined" &&
    new URLSearchParams(window.location.search).get("map") === "heatmap"
      ? "heatmap"
      : "choropleth",
  );
  ...
  const [nearbyFlyTarget, setNearbyFlyTarget] = useState<{
    lat: number;
    lng: number;
    login: string;
    zoom?: number;
  ...
  useEffect(() => {
    if (initialSummary !== undefined) return; // server pre-fetched — skip client fetch
    fetch("/api/explore")
  ...
  }, []); // initialSummary is stable after mount — intentionally excluded
  ...
  useEffect(() => {
    ...
    if (
      ...
      prev.search !== search ||
      prev.minFollowers !== minFollowers
    ) {
  ...
  useEffect(() => {
    ...
    if (nearbyCoords) {
      ...
      setNearbyFlyTarget({
        ...
        login: "__nearby_center__",
        zoom,
      });
    }
  }, [nearbyCoords, nearbyRadius]);

  const setPage = (t: Tab, p: number) =>
    setPages((prev) => ({ ...prev, [t]: p }));

  // Per-tab URLs
  const topUrl =
    tab === "top"
      ? buildUrl("/api/explore/top", {
          page: pages.top,
          size: PAGE_SIZE,
  ...
  const companiesUrl =
    tab === "companies"
      ? buildUrl("/api/explore/companies", {
  ...
  const countriesUrl =
    tab === "countries"
      ? buildUrl("/api/explore/locations", {
  ...
  const { data: topData, loading: topLoading } =
    useFetch<TopUsersResponse>(topUrl);
  const { data: powerData, loading: powerLoading } =
    useFetch<PowerResponse>(powerUrl);
  const { data: companiesData, loading: companiesLoading } =
    useFetch<CompaniesResponse>(companiesUrl);
  const { data: countriesData, loading: countriesLoading } =
    useFetch<LocationsResponse>(countriesUrl);
  const { data: citiesData, loading: citiesLoading } =
    useFetch<LocationsResponse>(citiesUrl);
  const { data: nearbyData, loading: nearbyLoading } =
    useFetch<NearbyResponse>(nearbyUrl);
  const { data: mapCountriesData, loading: mapCountriesLoading } =
    useFetch<LocationsResponse>(mapCountriesUrl);
  const { data: globalMapData, loading: globalMapLoading } =
    useFetch<GlobalMapData>(globalMapUrl);
  ...
  const showCountryFilter =
    tab !== "power" && tab !== "countries" && tab !== "nearby";
  ...
  const handleNearbyInputChange = useCallback(
    (v: string) => {
      setNearbyInput(v);
      setNearbyError(null);
  ...
  const handleNearbySearch = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/explore/geocode?q=${encodeURIComponent(q)}`,
      );
      if (res.status === 404) {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main
        id="main"
        className="flex-1 w-full max-w-7xl mx-auto px-4 lg:px-6 pt-6 pb-8"
      >
        {/* Page title */}
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted text-sm mt-1">
              Global leaderboards across all tracked repos
            </p>
          </div>
          <TourTrigger
            tourId="explore"
            label="Tour"
            className="flex-shrink-0 flex items-center gap-1.5 text-xs text-muted hover:text-foreground transition-colors px-2 py-1 rounded border border-border hover:border-accent-blue/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-blue/40 mt-1"
          />
        </div>

        {/* Summary cards */}
        <div
          data-tour="explore-summary"
          ...
          {(
            [
              ...
              },
              {
                ...
                value: summaryLoading
                  ? "…"
                  : (summary?.totalTrackedRepos ?? 0).toLocaleString(),
                accent: "text-foreground",
                icon: (
                  <svg
                    width="14"
              ...
              },
              {
                ...
                value: summaryLoading
                  ? "…"
                  : (summary?.totalCountries ?? 0).toLocaleString(),
            ...
            >
              ...
              <div
                className={`text-2xl font-bold tabular-nums leading-none ${accent}`}
              >
                {value}
              </div>
            </div>
            ...
            <div className="bg-surface border border-border rounded-xl overflow-hidden">
              ...
              {(showCountryFilter || showSearch) && (
                <div
                  ...
                  className="flex flex-col gap-2 px-5 pt-4 pb-0"
                >
                  ...
                  {tab === "top" && (
                    <div className="flex items-center gap-1.5">
                      <span className="text-2xs text-muted-subtle whitespace-nowrap">
                        Min followers
                      </span>
                      <div
                        role="radiogroup"
                        aria-label="Minimum followers filter"
              ...
              <div className="p-5">
                ...
                {tab === "top" && topLoading && topData && (
                  <div className="flex items-center gap-2 text-xs text-muted-subtle mb-3 px-2">
                    <svg
                      className="animate-spin size-3 text-accent-blue flex-shrink-0"
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <circle
                  ...
                  <>
                    <div className="flex items-start gap-2 bg-accent-orange/5 border border-accent-orange/20 rounded-lg px-3 py-2.5 mb-4">
                      <svg
                        ...
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        className="text-accent-orange flex-shrink-0 mt-px"
                        aria-hidden="true"
                      >
                        <polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" />
                      </svg>
                      <p className="text-xs text-muted leading-relaxed">
                        Developers who starred the most repos tracked on
                        StarMapper. The count shows how many of the{" "}
                        {(summary?.totalTrackedRepos ?? 0).toLocaleString()}{" "}
                        scanned repos they&apos;ve starred.
                      </p>
                    </div>
                    {powerData && (
                      <>
                        {powerData.items.length === 0 && (
                          <div className="text-center text-muted-subtle text-sm py-12">
                            ...
                            No power stargazers yet. Appears after multiple
                            repos are scanned.
                          </div>
                        )}
                        <div className="space-y-1">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="h-1 bg-surface-alt rounded-full">
                              <div
                                ...
                                style={{
                                  width: `${(count / (companiesData.items[0]?.[1] ?? 1)) * 100}%`,
                                }}
                              />
                            </div>
                          </div>
                ...
                {tab === "cities" && citiesData && (
                  <>
                    <StatsList
                      items={citiesData.items}
                      max={citiesData.items[0]?.[1] ?? 1}
                    />
                    <Pagination
                ...
                {tab === "nearby" && (
                  <div className="space-y-4">
                    ...
                    {!nearbyCoords && !nearbyError && (
                      <div className="text-center py-10 space-y-2">
                        ...
                        <p className="text-xs text-muted-subtle">
                          Based on public GitHub profiles. Locations may be
                          approximate.
                        </p>
                      </div>
                    )}
                    ...
                    {nearbyLoading && !nearbyData && (
                      <div className="text-center text-muted-subtle text-sm py-8">
                        Loading…
                      </div>
                    )}
                      ...
                      <>
                        ...
                        {nearbyData.users.length === 0 && (
                          <div className="text-center py-8 space-y-1">
                            ...
                            <p className="text-sm text-muted">
                              No developers found in this area.
                            </p>
                            <p className="text-xs text-muted-subtle">
                        ...
                        <p className="text-2xs text-muted-subtle border-t border-border-subtle pt-3 mt-2">
                          Based on public GitHub profile locations, which may be
                          approximate or outdated. Minimum radius 25km for
                          privacy.
                        </p>
                      </>
                    )}
                ...
                {showNearbyMap ? (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-2xs text-muted-subtle">
                      {nearbyData?.total ?? 0} developer
                      {(nearbyData?.total ?? 0) !== 1 ? "s" : ""} on map
                    </span>
                  </div>
                ) : (
                    ...
                    {nearbyLoading && !nearbyData && (
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <div className="bg-surface/80 backdrop-blur-sm border border-border rounded-xl px-4 py-2.5 flex items-center gap-2 shadow-lg">
                          <svg
                            className="animate-spin size-4 text-accent-blue"
                            viewBox="0 0 24 24"
                            fill="none"
                          >
                            <circle
