        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-6 py-10">
          <div className="flex items-end justify-between mb-5 gap-4">
            <div>
              ...
              <p className="text-sm text-muted mt-1">
                {loading
                  ? "Loading…"
                  : `${total.toLocaleString()} repositories scanned by the community`}
              </p>
            </div>
          </div>
          ...
          >
            ...
            <span className="flex-1 text-sm text-muted-subtle">
              {loading ? "Loading…" : `Search ${total.toLocaleString()} mapped repos...`}
            </span>
          ...
          {loading ? (
            <div className="flex flex-col gap-2">
              {Array.from({ length: 10 }).map((_, i) => (
                <div key={i} className="h-10 rounded-lg bg-surface animate-pulse motion-reduce:animate-none" />
              ))}
            </div>
          ) : (
