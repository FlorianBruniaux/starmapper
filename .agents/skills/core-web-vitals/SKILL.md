...
- MapLibre GL canvas itself doesn't cause CLS (fixed dimensions)
- Map popups injected via `innerHTML` can cause INP issues if DOM manipulation is heavy
- The chunk loop processes stargazers progressively — use `startTransition` for state updates during loop to avoid blocking interactions
- Stats drawer animations must use `transform`, never `height` transitions
