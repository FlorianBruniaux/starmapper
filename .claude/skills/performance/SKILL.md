...
- **MapLibre GL**: `maplibre-gl` is ~750KB. Always dynamic import with `ssr: false`. Never import at module level in Server Components.
- **Chunk loop**: use `startTransition` for React state updates during stargazer processing to keep UI responsive during long scans.
- **GeoJSON updates**: call `source.setData()` in batches, not on every chunk. Consider throttling updates to every 500 points.
- **Popup rendering**: MapLibre popups use `innerHTML` — keep content minimal to avoid layout thrashing.
- **Stats drawer**: lazy-load if it includes heavy charting libraries.
...
```bash
# Lighthouse CLI
npx lighthouse https://starmapper.bruniaux.com --output html --output-path report.html
