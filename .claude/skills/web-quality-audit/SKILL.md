...
1. **MapLibre GL bundle size** — dynamic import with `ssr: false`, never SSR
2. **Chunk loop INP** — React state updates during long scans block interactions
3. **Popup XSS** — `innerHTML` usage in MapLibre popups
4. **Dark mode contrast** — `@theme` tokens must meet WCAG AA on dark background
5. **OG images** — `/[owner]/[repo]/opengraph-image.tsx` must be correct size
6. **JSON-LD** — Structured data in `layout.tsx` for rich results
7. **GitHub token exposure** — Never in NEXT_PUBLIC_* variables
8. **Badge SVG caching** — CDN 6h TTL on `/api/badge/[owner]/[repo]`
